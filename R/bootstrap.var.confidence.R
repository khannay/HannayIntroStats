#' Function to find a confidence interval for the variance of a data set
#'
#' This function is useful for finding the confidence interval for the VARIANCE of a data set
#'
...
#' bootstrap.var.confidence(hkhw$Weight.lbs) #form the 95% confidence interval for the median weight of Hong Kong Children
#' @export
...
bootstrap.var.confidence <- function(mydata, conf.level=0.95, replicates=10000, alternative='two.sided', na.rm=TRUE) {
  #bootstap to get a confidence interval for the variance of this data set
  if (na.rm ) {
  ...
  resampled.values<-replicate(replicates, var(sample(mydata, numdatapoints, replace=TRUE)) )
