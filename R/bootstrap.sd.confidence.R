#' Function to find a confidence interval for the standard deviation of a data set
#'
#' This function is useful for finding the confidence interval for the standard deviation of a data set, for which the t.test command cannot be used.
#'
...
#' bootstrap.sd.confidence(hkhw$Weight.lbs) #form the 95% confidence interval for the median weight of Hong Kong Children
#' @export
...
bootstrap.sd.confidence <- function(mydata, conf.level=0.95, replicates=10000, alternative='two.sided', na.rm=TRUE) {
  #bootstap to get a confidence interval for the sd of this data set
  if (na.rm ) {
  ...
  resampled.values<-replicate(replicates, sd(sample(mydata, numdatapoints, replace=TRUE)) )
