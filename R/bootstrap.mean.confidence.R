#' Function to find a confidence interval for the mean of a data set
#'
#' This function is useful for finding the confidence interval for the mean of a data set, for which the t.test command cannot be used.
#'
...
#' bootstrap.mean.confidence(hkhw$Weight.lbs) #form the 95% confidence interval for the median weight of Hong Kong Children
#' @export
...
bootstrap.mean.confidence <- function(mydata, conf.level=0.95, replicates=10000, alternative='two.sided', na.rm=TRUE) {
  ...
  resampled.values<-replicate(replicates, mean(sample(mydata, numdatapoints, replace=TRUE)) )
